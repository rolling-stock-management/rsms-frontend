    ...
    <b-form v-if="show" @submit="onSubmit" @reset="onReset">
      <b-form-group id="inputName" label="Номер" label-for="inputName">
        <b-form-input
          id="inputName"
          v-model="form.number"
          placeholder="Въведете номер на локомотив..."
          required
        ></b-form-input>
      </b-form-group>
...
export default {
  ...
  meta: {
    auth: { permission: 'tractive-unit-create' },
  },
  data() {
    return {
      form: {
  ...
  computed: mapState({
    ...
    unitTypes: (state) => state.tractiveUnitTypes.tractiveUnitTypes,
  }),
  watch: {
    tractiveUnit(val) {
      this.$router.push({
    ...
    async onSubmit(event) {
      ...
      try {
        await this.$store.dispatch(
          'tractiveUnits/createTractiveUnit',
          this.form
        )
      } catch (error) {
        error({
          statusCode: 503,
          message: 'Неуспешно добавяне на локомотив. Моля, опитайте по-късно.',
        })
      }
    },
    onReset(event) {
      ...
      this.form.max_speed = ''
      this.form.power_output = ''
      this.form.weight = ''
      this.form.axle_arrangement = ''
      this.form.max_speed = ''
      this.form.length = ''
