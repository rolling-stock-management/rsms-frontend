...
export default {
  ...
  async fetch({ store, error }) {
    try {
      await store.dispatch('timetables/fetchTimetablesNoPagination')
    } catch (e) {
      error({
        statusCode: 503,
        message:
  ...
  methods: {
    async onSubmit(event) {
      ...
      try {
        await this.$store.dispatch('trains/createTrain', this.form)
      } catch (error) {
        error({
          statusCode: 503,
          message: 'Неуспешно добавяне на влак. Моля, опитайте по-късно.',
        })
      }
    },
    onReset(event) {
