  ...
  <div>
    ...
    <div v-if="toggle" class="input-group mb-3 bg-light mt-3 p-3 rounded">
      <div class="col-sm-12 col-md-4">
        <b-form-group
          label="Серия"
          label-for="selectWagonType"
          label-class="ml-2 pt-2 font-weight-bold text-primary"
        >
          <b-form-select id="selectWagonType" v-model="filters.wagonType">
            <b-form-select-option :value="null"
              >&lt; без филтър &gt;</b-form-select-option
            >
            <b-form-select-option
              v-for="(wagonType, id) in wagonTypes"
              :key="id"
              :value="wagonType.data.id"
              >{{ wagonType.data.name }}</b-form-select-option
            >
          </b-form-select>
        </b-form-group>
        <b-form-group
...
export default {
  props: {
    ...
    wagonTypes: { type: Array, required: true },
  },
  data() {
    return {
      filters: {
  ...
  methods: {
    filter: _.debounce(function (e) {
      ...
      if (this.filters.wagonType !== null) {
        this.queryString += '&type_id=' + this.filters.wagonType
      }
